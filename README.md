...
- **Extensible whitelist.** Add your own pipelines (OCR, `pdftolatex`, …) in `~/.config/verifelis/config.json` — same contract, human-predetermined:
  ...
  Entries are validated (fixed argv of strings, `<file>` exactly once, `<outdir>` at most once); malformed entries and missing binaries are reported at startup and by `/pipelines`, never offered to the model.
- **Index, not payload.** A pipeline whose argv contains `<outdir>` (e.g. `mineru`) writes to a derived-output directory that is auto-whitelisted for reading after the run. The tool result is an *index* — file list, main markdown path, heading outline with line numbers — so long conversions never flood the context; the cat then navigates with `grep` and line-ranged `read_file` (streamed, so ranges deep in large files stay reachable). Outputs are cached and reused while newer than the source.
